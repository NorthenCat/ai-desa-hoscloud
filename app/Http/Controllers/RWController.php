...
class RWController extends Controller
{
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($apiData['success']) {
                    ...
                    $localFailedFiles = \App\Models\FailedKkFile::where('rw_id', $localRwInstance->id)
                    ...
                    return view('rw.index', compact('rw', 'standaloneAnggota', 'anggotaTanpaNik', 'failedFiles', 'totalRT', 'localRwInstance'));
                }
            }
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($apiData['success']) {
                    ...
                    // Use LOCAL RW ID for redirect, not API RW ID
    ...
    public function exportExcel(Request $request, $desa_id, $rw_id)
    {
        try {
            ...
            // Web request handling (existing logic)
            $apiUrl = $this->getApiUrl();
            $apiToken = $this->getApiToken();
            $userId = $this->getUserId();
            ...
            if ($response->successful()) {
                // Return the file content from API
    ...
    public function exportExcelWithoutFilename(Request $request, $desa_id, $rw_id)
    {
        try {
            ...
            if ($response->successful()) {
                ...
                return response($response->body())
                    ->header('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
                    ->header('Content-Disposition', 'attachment; filename="export-no-filename.xlsx"');
            }
