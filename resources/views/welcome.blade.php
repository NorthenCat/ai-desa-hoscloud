...
<body
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                // Auto-setup database when the application loads
                setupDatabase();
            });
            ...
            async function setupDatabase() {
                try {
                    ...
                    if (statusData.success) {
                        ...
                        // If database is not connected or tables don't exist, run auto-setup
                        if (!status.database_connected || !status.tables_exist || !status.settings_seeded) {
                            console.log('Database needs setup, running auto-setup...');

                            const setupResponse = await fetch('/api/database/auto-setup');
                            const setupData = await setupResponse.json();

                            if (setupData.success) {
                                console.log('Database setup completed:', setupData.message);

                                // Show subtle notification
                                showNotification(setupData.message, 'success');
                            } else {
                                console.error('Database setup failed:', setupData.message);
                                showNotification('Database setup failed: ' + setupData.message, 'error');
                            }
                        } else {
                            console.log('Database already properly configured');
                        }
                    } else {
                        console.error('Failed to check database status:', statusData.message);
                    ...
                    showNotification('Failed to setup database: ' + error.message, 'error');
                }
            }
